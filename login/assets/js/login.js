(function ($) {
 ...
 function setLoading(isLoading) {
  ...
  // 获取剩余份数
  function getOrderNum(goods_id,max_member) {
    var tokens = JSON.parse(localStorage.getItem("userInfo")) || "";

    $.ajax({
      type: "get",
      url: `${baseUrl}/user/getordernum?goods_id=${goods_id}`,
      dataType: "json",
      headers: {
        Authorization: `Bearer ${tokens.token}`,
      },
      ...
      success: function (res) {
        if (res.code == 0) {
          if (id == 0) {
            ...
            var max_member = parseInt(res.data.list[0].max_member)
            getOrderNum(res.data.list[0].ID,max_member);
            $.each(res.data.list, function (index, data) {
              var type = "M";
              if (data.time == 10) {
                type = "M";
